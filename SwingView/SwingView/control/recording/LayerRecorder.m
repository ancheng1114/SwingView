...
- (CGContextRef)createBitmapContextOfSize:(CGSize)size {
	...
	context = CGBitmapContextCreate (NULL,
									 ...
									 bitmapBytesPerRow,
									 colorSpace,
									 (CGBitmapInfo)kCGImageAlphaNoneSkipFirst/*kCGImageAlphaPremultipliedFirst | kCGBitmapByteOrder32Little*/);
...
- (void)stopRecording {
    ...
    if (mAssetWriterInput)
    {
        ...
        int status = mAssetWriter.status;
        while (status == AVAssetWriterStatusUnknown) {
            [NSThread sleepForTimeInterval:0.2];
            status = mAssetWriter.status;
        }
